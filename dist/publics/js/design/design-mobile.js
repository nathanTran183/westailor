...
function renderImage(component_id, componentImg, product_id) {
    if (!$('#' + product_id + ' .slide.layers.front').find('#' + component_id).attr('src')) {
        if (!!componentImg.img_url_front) $('#' + product_id + ' .slide.layers.front').append('<img src="' + componentImg.img_url_front + '" alt="front" id="' + component_id + '" class="front">');
    } else {
        if (!!componentImg.img_url_front) $('#' + product_id + ' .slide.layers.front .front#' + component_id).attr('src', componentImg.img_url_front);else $('#' + product_id + ' .slide.layers.front .front#' + component_id).remove();
    }
    if (!$('#' + product_id + ' .slide.layers.back').find('#' + component_id).attr('src')) {
        if (!!componentImg.img_url_back) $('#' + product_id + ' .slide.layers.back').append('<img src="' + componentImg.img_url_back + '" alt="back" id="' + component_id + '" class="back">');
    } else {
        if (!!componentImg.img_url_back) $('#' + product_id + ' .slide.layers.back .back#' + component_id).attr('src', componentImg.img_url_back);else $('#' + product_id + ' .slide.layers.back .back#' + component_id).remove();
    }
}
...
slides_generator = function () {
    ...
    t.changeView = function (t, e) {
        if (e = void 0 !== e && e, "without_jacket" == t) {
            i = this.view;
            return this.view = "front", $("#available_window").removeClass(i), $("#available_window").addClass("front"), $("#available_window .views li.active").removeClass("active"), $('#available_window .views li[rel="without_jacket"]').addClass("active");
        }
    ...
    if (navigator.msMaxTouchPoints) e.addClass("ms-touch");else {
        t.slider = {
            ...
            change_view: function () {
                i.removeClass("visible").filter(".index_" + this.index).addClass("visible"), t.changeView(t.available_views[this.index], !0);
...
$(document).ready(function () {
    ...
    $('.filter_button').click(function () {
        ...
        if (counter == 0) $('#empty_list_fabric').addClass('active');else $('#empty_list_fabric').removeClass('active');
    });
    ...
    $('.garment_block_extra .box_opt_fix label').click(function () {
        ...
        if ($(this).attr('rel') == "1") $('.garment_block_extra #measure_form').slideUp();else $('.garment_block_extra #measure_form').slideDown();
    });
    ...
    $('.garment_block_extra #measure_form input').keyup(function () {
        ...
        } else input.css('border', '1px solid #13bb18');
    });

    $('.action .add_to_cart').click(function () {
        let measureVal = $('.garment_block_extra .box_opt_fix .checked').attr('rel');
        currentDesign.products.map(product => {
            delete product.img;return product;
        });
        ...
        if (measureVal == "0") {
            ...
            currentDesign.quantity = 1;
            currentDesign.products.map(product => {
                delete product.measure;return product;
            });
            if (!pos) {
                currentDesign.quantity = 1;
            } else {
