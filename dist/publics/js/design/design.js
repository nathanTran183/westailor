...
function renderImage(component_id, componentImg, product_id) {
    if (!$('.images_render #' + product_id).find('#' + component_id).attr('src')) {
        ...
        } else $('.images_render #' + product_id + ' .front#' + component_id).remove();
        if (!!componentImg.img_url_back) $('.images_render #' + product_id + ' .back#' + component_id).attr('src', componentImg.img_url_back);else $('.images_render #' + product_id + ' .back#' + component_id).remove();
    }
...
$(document).ready(function () {
    ...
    $('.filters.season .background_fixed a').click(function () {
        ...
        if (counter == 0) $('.empty_fabrics').addClass('active');else $('.empty_fabrics').removeClass('active');
        $('.filters.season').removeClass('active');
    });
    ...
    $('#extra .checkbox_option label').click(function () {
        ...
        if ($(this).attr('rel') == "1") $('#extra #measure_form').slideUp();else $('#extra #measure_form').slideDown();
    });
    ...
    $('#extra #measure_form input').change(function (event) {
        if (isNaN(+$(this).val()) || +$(this).val() <= 0) $(this).css('border', '1px solid #ff0000');else $(this).css('border', '1px solid #13bb18');
    });
