...
module.exports = {
    ...
    get: async (req, res) => {
        try {
            ...
            if (!discount) return res.json({ status: false, message: "Discount coupon does not exist!" });
    ...
    checkDiscount: async (req, res) => {
        try {
            ...
            if (new Date(discount.date_start) > new Date() || expireDate < new Date()) return res.status(403).json({ status: false, message: "Invalid discount coupon time! Please make a check!" });
