...
module.exports = {
    listOrder: async (req, res) => {
        try {
            let type = req.query.status,
                orders;
            if (!type || !!type && type == 'pending') orders = await Order.find({ user_id: req.session.user.id, status: { $in: [1, 2] } }).sort({ createdAt: 'desc' }).lean();else if (type == 'process') orders = await Order.find({ user_id: req.session.user.id, status: { $in: [3, 4] } }).sort({ createdAt: 'desc' }).lean();else if (type == 'history') orders = await Order.find({ user_id: req.session.user.id }).sort({ createdAt: 'desc' }).lean();
    ...
    getOrder: async (req, res) => {
        try {
            ...
            let order = await Order.findOne({ user_id: req.session.user.id, _id: id }).populate({ path: 'order_item', select: "_id quantity item_id price products" }).lean();
    ...
    getSessionCart: async (req, res) => {
        try {
            ...
            let itemData = JSON.parse(itemFile);
            let carts = [];
            if (!!req.session.carts) carts = JSON.parse(JSON.stringify(req.session.carts));
                ...
                if (result.success) {
                    ...
                    for (var i = 0; i < req.session.carts.length; i++) {
                        ...
                        cart.products.forEach(product => {
                            if (!product.measure) order.status = 1;else cart.status = true;
                        });
    ...
    postCreate: async (req, res) => {
        try {
            ...
            if (!!req.body.search) {
                ...
                if (!!user) order.user_id = user._id;else {
